...
uint8_t *pkcs7_unpad(uint8_t *data, uint32_t *len_addr)
{
    ...
    uint8_t *new_data = new uint8_t[(len - padding)];
...
{
    uint8_t *new_data = reinterpret_cast<uint8_t *>(malloc(sizeof(uint8_t) * (strlen(reinterpret_cast<const char *>(data)) - 17)));
    memcpy(new_data, data + 17, strlen(reinterpret_cast<const char *>(data)) - 17);
    free(data);
    return new_data;
}
              ...
              uint32_t *music_file_size, char **filename)
{
    ...
    uint8_t *key_data_array = new uint8_t[key_length];
    ...
    uint8_t *key_box = new uint8_t[256];
    ...
    uint8_t *meta_data = new uint8_t[meta_length];
    ...
    uint8_t *meta_decode_data = b64_decode_ex(reinterpret_cast<const char *>(meta_data), meta_length, &meta_decode_data_len); // base64解码后数据
    ...
    cJSON *json = cJSON_Parse(reinterpret_cast<const char *>(meta_data_json));
    ...
    char *music_file_name = new char[strlen(music_name) + strlen(music_format) + 2];
    ...
    *filename = music_file_name; // 返回真实文件名
    ...
    uint8_t *music_data = new uint8_t[music_size];
...
int work_convert(char *input_path)
{
...
#else
    HANDLE fd = CreateFile(input_path,
                           //GENERIC_READ,
    ...
    size_t filesize = st.nFileSizeLow | (static_cast<size_t>(st.nFileSizeHigh) << 32);

    HANDLE hMapping = CreateFileMapping(fd,
                                        nullptr,
                                        // PAGE_READONLY,
                                        PAGE_READWRITE,
                                        st.nFileSizeHigh,
    ...
    if (hMapping == NULL) {
        DWORD dwError = GetLastError();
        std::cout << "hMapping = " << (dwError) << std::endl;
        return -1;
    }
#endif
...
#else
    void *filedata = MapViewOfFile(hMapping, FILE_MAP_ALL_ACCESS, 0,0,0);
    if (nullptr == filedata) {
        DWORD dwError = GetLastError();
        std::cout << "filedata = " << (dwError) << std::endl;
        return -1;
    }
#endif
    ...
    uint8_t *decode_file_data = dump(reinterpret_cast<uint8_t *>(filedata), filesize,
